        ...
        {relatedPdfs.map((paper, index) => (
          <>
            <li key={index} className="p-4 bg-card rounded-lg shadow">
              ...
              <Button onClick={() => handleButtonClick(paper.title, paper.url)} className="whitespace-nowrap font-semibold bg-black mr-2 p-2 rounded-md text-white text-sm">
                Chat with the paper
              </Button>
              <Link href={`${paper.url}`} className="whitespace-nowrap font-semibold bg-black p-2 py-[10px] rounded-md text-white text-sm">
                Download
              </Link>
            </li>
          </>
